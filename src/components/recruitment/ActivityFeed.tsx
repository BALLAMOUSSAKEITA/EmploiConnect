...
function formatAction(action: string, meta: Entry["meta"]): string {
  ...
  if (action === "team_member_added" && meta && typeof meta === "object") {
    const m = meta as Record<string, string>;
    const roleFr: Record<string, string> = {
      lead_recruiter: "Recruteur principal",
      sourcer: "Sourcing",
      coordinator: "Coordinateur",
      hiring_manager: "Manager / Décideur",
    };
    const r = roleFr[m.role] ?? m.role;
    return `Co-recrutement : ${m.name || "Collaborateur"} ajouté (${r})`;
  }
  if (action === "team_member_removed" && meta && typeof meta === "object") {
    const m = meta as Record<string, string>;
    const roleFr: Record<string, string> = {
      lead_recruiter: "Recruteur principal",
      sourcer: "Sourcing",
