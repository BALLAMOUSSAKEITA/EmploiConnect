...
import { ArrowLeft, Phone, Mail, MapPin, Briefcase, FileText, Upload, Plus, History, Tag, Layers, BellRing, X } from "lucide-react";
...
export default function CandidateDetailPage() {
  ...
  const [talentLists, setTalentLists] = useState<{ id: number; name: string }[]>([]);
  const [vivTags, setVivTags] = useState<string[]>([]);
  const [vivTagDraft, setVivTagDraft] = useState("");
  const [vivRecontactAt, setVivRecontactAt] = useState("");
  const [vivRecontactNote, setVivRecontactNote] = useState("");
  const [vivListIds, setVivListIds] = useState<number[]>([]);
  const [vivSaving, setVivSaving] = useState(false);

  useEffect(() => {
    api.get<{ id: number; name: string }[]>("/talent-lists").then((r) => setTalentLists(r.data)).catch(() => setTalentLists([]));
  }, []);

  useEffect(() => {
    if (!candidate?.id) return;
    setVivTags([...(candidate.tags || [])]);
    setVivRecontactAt(toDatetimeLocal(candidate.recontact_at));
    setVivRecontactNote(candidate.recontact_note || "");
    setVivListIds((candidate.talent_lists || []).map((x: { id: number }) => x.id));
  }, [candidate?.id]);
  ...
  const addVivTag = () => {
    const t = vivTagDraft.trim();
    if (!t) return;
    if (vivTags.some((x) => x.toLowerCase() === t.toLowerCase())) {
      setVivTagDraft("");
      return;
    }
    setVivTags([...vivTags, t]);
    setVivTagDraft("");
  };

  const saveVivier = async () => {
    setVivSaving(true);
    try {
      const { data } = await api.put(`/candidates/${id}`, {
        tags: vivTags,
        recontact_at: vivRecontactAt ? new Date(vivRecontactAt).toISOString() : null,
        recontact_note: vivRecontactNote.trim() || null,
        talent_list_ids: vivListIds,
      });
      setCandidate(data);
      toast("Vivier mis à jour", "success");
    } catch (e: unknown) {
      const msg =
        typeof e === "object" && e !== null && "response" in e
          ? (e as { response?: { data?: { detail?: string } } }).response?.data?.detail
          : undefined;
      toast(typeof msg === "string" ? msg : "Impossible d’enregistrer", "error");
    } finally {
      setVivSaving(false);
    }
  };
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
              <h3 className="font-semibold text-slate-800 flex items-center gap-2 text-sm">
                ...
                Vivier (tags, listes, rappel)
              </h3>
              <Button size="sm" type="button" onClick={saveVivier} disabled={vivSaving}>
                {vivSaving ? "…" : "Enregistrer"}
              </Button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Tags</label>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {vivTags.map((t) => (
                    <span
                      key={t}
                      className="inline-flex items-center gap-1 bg-amber-50 text-amber-900 text-xs px-2 py-1 rounded-full border border-amber-100"
                    >
                      <Tag className="w-3 h-3 opacity-70" />
                      {t}
                      <button
                        type="button"
                        onClick={() => setVivTags((prev) => prev.filter((x) => x !== t))}
                        className="p-0.5 rounded hover:bg-amber-100 text-amber-800"
                ...
                <div className="flex gap-2">
                  <input
                    value={vivTagDraft}
                    onChange={(e) => setVivTagDraft(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addVivTag())}
                    placeholder="Ajouter un tag (Entrée)"
                    className="flex-1 text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500/25"
                  />
                  <Button type="button" size="sm" variant="secondary" onClick={addVivTag}>
                    Ajouter
                  </Button>
                </div>
              </div>
              <div>
                ...
                <input
                  type="datetime-local"
                  value={vivRecontactAt}
                  onChange={(e) => setVivRecontactAt(e.target.value)}
                  className="w-full max-w-xs text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-500/25 bg-white mb-2"
                />
                <textarea
                  value={vivRecontactNote}
                  onChange={(e) => setVivRecontactNote(e.target.value)}
                  placeholder="Note pour la prochaine prise de contact…"
                  rows={2}
                  className="w-full text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-500/25 resize-y"
                />
                <p className="text-[11px] text-slate-400 mt-1">Laissez la date vide pour supprimer le rappel après enregistrement.</p>
              </div>
              <div>
                <label className="block text-[11px] font-medium text-slate-500 mb-2">Listes du vivier</label>
                {talentLists.length === 0 ? (
                  <p className="text-xs text-slate-400">Aucune liste — créez-en depuis la page Candidats.</p>
                ) : (
                    ...
                    {talentLists.map((l) => (
                      <li key={l.id}>
                        <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={vivListIds.includes(l.id)}
                            onChange={() =>
                              setVivListIds((prev) =>
